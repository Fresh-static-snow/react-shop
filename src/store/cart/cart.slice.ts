...
export const cartSlice = createSlice({
   ...
   initialState,
   reducers: {
      addProduct(state: CartState, action: PayloadAction<CartItem>) {
      ...
      increaseCount(state: CartState, action: PayloadAction<CartItem>) {
      ...
      decreaseCount(state: CartState, action: PayloadAction<CartItem>) {
         state.cartItems = state.cartItems.map((item) => {
            if (JSON.stringify(item) === JSON.stringify(action.payload)) {
               item.quantity -= 1
               return item
      ...
      toggleItemAdded(state: CartState, action: PayloadAction<boolean>) {
         state.itemAdded = action.payload
      },
      toggleItemRemoved(state: CartState, action: PayloadAction<boolean>) {
